...
class Settings(dict):
        ...
        def info(self):
            print(
                '{}:\n\n{}. Suggested value/range:{}\n'.format(
                    self.name, self.message, self.suggestedValue
                )
            )
    ...
    _valid_options = {
        ...
        'refStruct': Option(
            ...
            'The database key corresponding to the name of the structure that'\
                'will be used as the reference structure for energy difference'\
                    'calculations (i.e. E = E_struct - E_ref).'
        ),
        'databasePath': Option(
        ...
        with open(filePath, 'r') as settingsFile:
            ...
            for line in lines:
                ...
                if (len(line) == 0) or (line[0] == '#'):
                    ...
                    if key not in cls._valid_options:
                        raise RuntimeError(
                            'Invalid option: {}'.format(key)
                        )

                    option = cls._valid_options[key]
    ...
    def from_dict(cls, settingsDict):
        ...
        for expectedKey in cls._valid_options:
            if expectedKey not in settings:
                value = cls._valid_options[key].suggestedValue
    ...
    def printValidSettings(self):
        for key, option in self._valid_options.items():
            print(
                '{} ({}):\n\n{}\n'.format(
                    key, option.allowedType, option.info()
                )
            )

    def printSettings(self):
        for key, val in self.items():
            option = self._valid_options[key]
